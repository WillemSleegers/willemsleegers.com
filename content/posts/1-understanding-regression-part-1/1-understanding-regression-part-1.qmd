...
description: "This is the first post in a series on understanding regression. I start with a brief introduction to the importance of understanding regression well and then discuss some data and ask the question what we should ask of this data, as the first step to understanding regression."
date: 2025-01-15
tags:
  - statistics
  - tutorial
...
We get an estimate of `r round(coef(model)[[1]], 2)` cm with a standard error of `r round(summary(model)$coefficients[, "Std. Error"], 2)` cm. 
...
- Estimate what the typical value is and how uncertain that estimate is
- Test hypotheses (does the mean differ from some value?)
