...
as_draws_df(test_reg1)

tibble(male = c(0, 1)) %>%
  add_epred_draws(model_gender_dummy, value = "predicted_height") %>%
  median_qi() %>%
  mutate(width = .upper - .lower)

ggplot(dummy_heights, aes(x = predicted_height, fill = factor(male))) +
