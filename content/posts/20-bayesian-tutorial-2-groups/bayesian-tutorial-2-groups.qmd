...
<<<<<<< Updated upstream
In this blog post I will cover how to use `brms` to analyze the difference between two groups. Interestingly, this might seem like a very simple analysis, but there are actually many ways to go about this.
=======
...
The R formula for regressing height on male is `height ~ male`. Let's see what priors we need to set for this by using by using `get_prior()`.
=======
Probably the most common method to analyze the difference between two groups is to regress the outcome on dummy coded data. That means the group column contains the information about both groups, usually using a 0 and 1 to indicate which group is which. The `male` column in the current data frame is dummy coded, with a 0 for females and a 1 for males. The R formula for regressing height on male is `height ~ male`. Let's see what priors we need to set for this.
>>>>>>> Stashed changes
...
=======
The output shows we need to set a prior on sigma, the Intercept, and on the male coefficient. The intercept refers to the heights of female participants and the male coefficient refers to the difference between males and females, at least that's what you would except in standard regression. We'll see that things are a bit more complicated. Sigma refers, as always, to the standard deviation of the residuals.
>>>>>>> Stashed changes
...
Now the estimate of the intercept is indeed 165, as indicated by our prior. Let's run the model again so we also get the posterior results.
...
The estimates show that the average height for women is `r round(fixef(model_dummy)[1, 1], 2)` and that for men is `r round(fixef(model_dummy)[1, 1], 2)` + `r round(fixef(model_dummy)[2, 1], 2)` = `r round(fixef(model_dummy)[1, 1] + fixef(model_dummy)[2, 1], 2)`.
...
Next, we run the model with the new priors. I simply translated the previous priors to this new notation.
...
The results now show estimates for both female and male participants directly. Excellent. If you want to verify that the width of the 95% quartile range is the same for both sexes (because the standard deviation of the normal prior for each sex is the same), you can re-run the previous model and sample only from the prior, like in the previous section).

## Calculating a difference score

Seeing the estimates for each sex is nice and all, but oftentimes we're interested in determining whether there's a difference between the two groups. This is easy to do when using index coding because we can simply take the posterior samples from each group and subtract them from each other.
