...
ggplot(draws, aes(x = b_Intercept)) +
  stat_slab() +
  stat_dotsinterval(side = "bottom", .width = .95, alpha = .5)
...
Looks like a sizeable difference between the two sexes, which is not at all surprising of course. Women and children were put in lifeboats first, leading to more women surviving than men. We can calculate the size of this difference by simple subtracting the two estimates and converting it to a proportion.
