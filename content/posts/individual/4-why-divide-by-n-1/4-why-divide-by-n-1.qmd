...
#| fig-cap: Average biased variance by sample size
...
#| fig-cap: Average unbiased variance by sample size
...
#| fig-cap: Deviations from the sample mean (sample 1)
...
#| fig-cap: Deviations from the population mean (sample 1)
ggplot(sample1, aes(y = index, x = value, color = sample)) +
  geom_point(size = 2.5) +
  geom_vline(xintercept = mean_sample, linetype = "dashed") +
  geom_vline(xintercept = mean_population, linetype = "solid") +
...
#| fig-cap: Deviations from the sample mean (sample 2)
...
#| fig-cap: Deviations from the population mean (sample 2)
ggplot(sample2, aes(y = index, x = value, color = sample)) +
  geom_point(size = 2.5) +
  geom_vline(xintercept = mean_sample, linetype = "dashed") +
  geom_vline(xintercept = mean_population, linetype = "solid") +
...
#| fig-cap: Sources of variance by sample size
...
The variance of a sample will be biased by an amount equal to $\frac{n - 1}n$. To correct that bias we should multiply the sample variance by the inverse of this bias: $\frac{n}{n-1}$ [^3]. This is also called [Bessel's correction](https://en.wikipedia.org/wiki/Bessel%27s_correction).
