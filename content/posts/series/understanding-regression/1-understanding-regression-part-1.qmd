...
Let's immediately run a regression so we have some numbers to look at. In these first posts, we'll try to figure out where these numbers come from.
...
As you can see in the output, we get an estimate of `r round(coef(model)[[1]], 2)` cm, a standard error of `r round(summary(model)$coefficients[, "Std. Error"], 2)` cm, a t-value of `r round(summary(model)$coefficients[, "t value"], 1)`, and a p-value of `r format(summary(model)$coefficients[, "Pr(>|t|)"], scientific = TRUE, digits = 2)`.
...
The dashed line is the normal distribution we're proposing heights are drawn from. As we'll see, this is what `lm()` estimates. 
