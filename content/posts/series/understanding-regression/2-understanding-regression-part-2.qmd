...
In my own education I often heard that the normal distribution is an assumption. It's something you assume when modelling and then you are supposed to verify that assumption. However, I want to push back against that idea a little bit because it sounds as if you don't have good reasons to assume a normal distribution. However, we can often use prior knowledge about a phenomena to argue plausibly that a normal distribution is a good description of the data (e.g., when you know it is the result of many small effects). 

Another argument to push back against the idea of the normal distribution being an assumption is that it's a very conservative model.
