...
Let's be precise about what SE means. It is the standard deviation of the sampling distribution. If you could draw many independent samples of the same size, calculate the mean each time, and then take the standard deviation of all those means, that's the SE. Our simulation did exactly this.
...
The dashed line in the plot shows the formula that captures this relationship: SE = σ / √n.^[The √n reflects the diminishing returns we see in the plot. But where does it come from? Think about drawing n heights from our normal distribution and computing their mean. Each height varies with standard deviation σ. How much does the mean vary?

To answer this, we need to work with variances (σ²) rather than standard deviations (σ). The reason is that variances of independent draws add up, while standard deviations don't (the next footnote explains why).

So: each height has variance σ². Adding them together means we multiply by the sample size: n × σ². To get the mean, we divide by n. Because variance is on a squared scale, dividing by n shrinks the variance by n² (not n), giving n × σ² / n² = σ² / n. Taking the square root to get back to the original scale (centimeters, not centimeters squared): √(σ² / n) = σ / √n. The √n in the denominator is why doubling the sample size doesn't halve the SE. You'd need to quadruple it.]^[But why do variances add while standard deviations don't? Imagine just two heights, each varying by σ around the mean. There are four equally likely scenarios: both above average (sum is 2σ above expected), first above and second below (they cancel out), first below and second above (they cancel out again), and both below average (sum is 2σ below expected). If deviations always pointed the same direction, the sum would vary by 2σ. But half the time they point in opposite directions and cancel out. Work out the standard deviation of those four scenarios and you get σ√2, not 2σ. That's the pattern. With n independent heights, the sum varies by σ√n, not nσ. The cancellation is why adding standard deviations overshoots. Variances are the quantity where this cancellation is already accounted for, so they simply add up.]
