...
Like μ, σ is estimated from our sample, and a different sample would give a different estimate. This uncertainty about the estimate matters since σ feeds directly into calculating the SE for μ. If our estimate of σ is off, our measure of uncertainty about μ is off too. 
...
At n = 10, the sample SD is all over the place. Some samples produce SDs below 4, others above 12, even though we're drawing from a distribution with σ = `r round(sample_sd, 1)`. At n = 50, the spread narrows considerably. At n = 352 (our actual sample), we're fairly precise.

## A different shape

Compare these distributions to the sampling distribution of the mean from Part 4. The mean's sampling distribution was symmetric: a normal distribution centered on μ. The SD's sampling distribution looks different. It's slightly **skewed**, with the right tail extending further than the left. This is especially visible at n = 10.
...
With larger samples, the skew fades. At n = 352, the distribution is nearly symmetric. But at small sample sizes, the asymmetry is real and has consequences. This convergence is the same principle we saw in Part 4: with more data, sampling distributions tend toward normality. The chi-squared distribution is no exception.^[The Central Limit Theorem again.]
...
where Γ is the **gamma function**.^[The formula comes from the fact that the sample variance follows a scaled chi-squared distribution. The gamma function ratios describe the exact mean and variance of the resulting chi distribution (the distribution of the sample SD). Unlike SE(μ) = σ / √n, this doesn't simplify to anything tidy.] I won't pretend this formula is helpful, it's gibberish to me, but what matters is that it matches our simulations and that we can use it to calculate the standard error of sigma exactly. 
...
While we see the standard error for the intercept, we don't see a standard error for the residual standard error. At first glance that's not surprising — why would you need a standard error of a standard error? But as we've discussed, the residual standard error is our estimate of σ, which carries its own uncertainty. This uncertainty matters because it feeds into the uncertainty of other estimates, like our estimate of μ. When we estimate σ rather than knowing it, we introduce extra uncertainty into the SE itself.

So why is that not included in the output of `lm()`?

The reason is that the frequentist framework [what is the frequentist framework? We haven't explained this at all. And is this truly something that applies to the frequentist framework? my understanding of frequentism is that it is about using long-running probabilities or something like that (as in, that is the crucial concept in it)] handles σ's uncertainty *implicitly*. Rather than reporting σ's uncertainty explicitly, `lm()` uses the t-distribution instead of the normal when computing p-values and confidence intervals for μ. The t-distribution's heavier tails absorb exactly the extra uncertainty that comes from estimating σ rather than knowing it. You never see σ's uncertainty in the output, but it's baked into the distribution used for inference.

[at this point we have covered the estimation goal. Do we make it sufficiently clear we've done that?]
...
In Part 6, we'll see how σ's uncertainty shapes the inference we draw about μ.