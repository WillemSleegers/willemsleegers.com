...
But look at that formula. It uses σ. We don't know σ. We estimated it from our sample, just like we estimated μ. So how much does our estimate of σ vary?
...
So why doesn't `lm()` report a standard error for σ? Partly convention: in most research questions, σ is a nuisance parameter, something you need to estimate in order to do inference about μ, but not a quantity researchers typically ask about directly. But there's also a deeper reason. The frequentist framework handles σ's uncertainty *implicitly*. Rather than reporting it, `lm()` uses the t-distribution instead of the normal when computing p-values and confidence intervals for μ. The t-distribution's heavier tails absorb exactly the extra uncertainty that comes from estimating σ rather than knowing it. You never see σ's uncertainty in the output, but it's baked into the distribution used for inference. That's what Part 6 is about.
